...
const deriveChartTypeIds = (chartTypes: Array<{ id: number; name: string }>) => {
  ...
  const pickByExactName = (name: string) =>
    chartTypes.find(
      (item) => item.name.trim().toLocaleLowerCase("ru-RU") === name.toLocaleLowerCase("ru-RU"),
    )?.id ?? null;
  const pickByNameLike = (needle: string) =>
    chartTypes.find((item) => includesWord(item.name, needle))?.id ?? null;
  return {
    horizontalId:
      pickByExactName("Горизонтальный") ?? pickByNameLike("горизонт") ?? sorted[0]?.id ?? null,
    verticalId:
      pickByExactName("Вертикальный") ?? pickByNameLike("вертик") ?? sorted[1]?.id ?? null,
...
export function DashboardsPage({ currentUser }: DashboardsPageProps) {
  ...
  const managerOptions = useMemo(
    () =>
      Array.from(new Set(deals.map((deal) => deal.managerName))).sort((a, b) =>
  ...
  const lifecycleOptions = useMemo(
    () =>
      Array.from(new Set(deals.map((deal) => deal.dealLifecycleStatusName))).sort((a, b) =>
        a.localeCompare(b, "ru-RU"),
      ),
    [deals],
  );
  ...
  const renderManagerSelect = (chartKey: string) => (
    <select
      ...
      onChange={(event) =>
        setManagerFilters((prev) => ({ ...prev, [chartKey]: event.target.value }))
      }
  ...
  const renderLifecycleSelect = (chartKey: string, includeAllOption = true) => (
    <select
      ...
      onChange={(event) =>
        setLifecycleFilters((prev) => ({ ...prev, [chartKey]: event.target.value }))
      }
  ...
  const renderChartActions = (
    chartKey: string,
    includeManagerFilter = true,
    includeLifecycleFilter = true,
    includeAllLifecycleOption = true,
  ) => (
    <>
  ...
  const activeDeals = useMemo(
    () =>
      periodFilteredDeals.filter((deal) => includesAny(deal.dealLifecycleStatusName, ["актив"])),
    [periodFilteredDeals],
  );
  const realizedDeals = useMemo(
    () =>
      periodFilteredDeals.filter((deal) => includesAny(deal.dealLifecycleStatusName, ["реализ"])),
    [periodFilteredDeals],
  );
  const delayedDeals = useMemo(
    () =>
      periodFilteredDeals.filter((deal) => includesAny(deal.dealLifecycleStatusName, ["отлож"])),
    [periodFilteredDeals],
  );
  const failedDeals = useMemo(
    () =>
      periodFilteredDeals.filter((deal) => includesAny(deal.dealLifecycleStatusName, ["несост"])),
    [periodFilteredDeals],
  );
  ...
  const hotDealsCount = useMemo(
    () => activeDeals.filter((deal) => includesAny(deal.dealStatusName, ["горяч"]))?.length ?? 0,
    [activeDeals],
  );
  const coldDealsCount = useMemo(
    () => activeDeals.filter((deal) => includesAny(deal.dealStatusName, ["холод"]))?.length ?? 0,
    [activeDeals],
  );
  ...
  const managerCountSegments = useMemo(() => {
    ...
    for (const deal of periodFilteredDeals) {
      if (
        managerCountLifecycleFilter &&
        deal.dealLifecycleStatusName !== managerCountLifecycleFilter
      )
        continue;
  ...
  const managerIncomeSegments = useMemo(() => {
    ...
    for (const deal of periodFilteredDeals) {
      if (
        managerIncomeLifecycleFilter &&
        deal.dealLifecycleStatusName !== managerIncomeLifecycleFilter
      )
        continue;
      sums.set(
        deal.managerName,
  ...
  const stageSegments = useMemo(() => {
    ...
    for (const deal of periodFilteredDeals) {
      ...
      if (stageAvLifecycleFilter && deal.dealLifecycleStatusName !== stageAvLifecycleFilter)
        continue;
  ...
  const leasingSegments = useMemo(() => {
    ...
    for (const deal of periodFilteredDeals) {
      ...
      if (leasingAvLifecycleFilter && deal.dealLifecycleStatusName !== leasingAvLifecycleFilter)
        continue;
  ...
  const managerCountMax = useMemo(
    () => Math.max(0, ...managerCountSegments.map((s) => s.value)),
    [managerCountSegments],
  );
  const managerIncomeMax = useMemo(
    () => Math.max(0, ...managerIncomeSegments.map((s) => s.value)),
    [managerIncomeSegments],
  );
  const stageAvMax = useMemo(
    () => Math.max(0, ...stageSegments.map((s) => s.value)),
    [stageSegments],
  );
  const leasingAvMax = useMemo(
    () => Math.max(0, ...leasingSegments.map((s) => s.value)),
    [leasingSegments],
  );
  ...
  return (
    <div className={styles.page}>
      ...
      <div className={styles.metricsGrid}>
        ...
        <Card title="Горячие сделки" subtitle="По активным сделкам" className={styles.metricCard}>
          {isLoading ? (
            <Spinner size={22} />
          ) : (
            <div className={styles.metricValueHot}>{formatNumberLike(hotDealsCount)}</div>
          )}
        </Card>
        <Card title="Холодные сделки" subtitle="По активным сделкам" className={styles.metricCard}>
          {isLoading ? (
            <Spinner size={22} />
          ) : (
            <div className={styles.metricValueCold}>{formatNumberLike(coldDealsCount)}</div>
          )}
        </Card>
        <Card title="Заработок" subtitle="По реализованным сделкам" className={styles.metricCard}>
          {isLoading ? (
            <Spinner size={22} />
          ) : (
        ...
        <Card
          ...
          subtitle="По несостоявшимся сделкам"
          className={styles.metricCard}
        >
          {isLoading ? (
            <Spinner size={22} />
        ...
        <Card
          title="Доход в отложенных сделках"
          subtitle="По отложенным сделкам"
          className={styles.metricCard}
        >
          {isLoading ? (
            <Spinner size={22} />
