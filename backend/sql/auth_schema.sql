...
CREATE TABLE password_setup_tokens (
  ...
  kind TEXT NOT NULL CHECK (kind IN ('invite', 'owner_reset')),
  expires_at TIMESTAMPTZ NOT NULL,
  used_at TIMESTAMPTZ NULL,
  issued_by_user_id BIGINT NULL REFERENCES users(id) ON DELETE SET NULL,
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
CREATE INDEX idx_password_setup_tokens_issued_by_user_id ON password_setup_tokens(issued_by_user_id);
